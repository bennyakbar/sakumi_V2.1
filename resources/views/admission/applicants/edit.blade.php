<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('app.nav.applicants') }} - {{ __('app.button.edit') }}
        </h2>
    </x-slot>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="admission_period_id" name="admission_period_id"
                                    ...
                                    @foreach($periods as $period)
                                        <option value="{{ $period->id }}" {{ old('admission_period_id', $applicant->admission_period_id) == $period->id ? 'selected' : '' }}>{{ $period->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="target_class_id" name="target_class_id"
                                    ...
                                    @foreach($classes as $class)
                                        <option value="{{ $class->id }}" {{ old('target_class_id', $applicant->target_class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="category_id" name="category_id"
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_id', $applicant->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="previous_school" class="block mt-1 w-full" type="text" name="previous_school"
                                    :value="old('previous_school', $applicant->previous_school)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="parent_name" class="block mt-1 w-full" type="text" name="parent_name"
                                    :value="old('parent_name', $applicant->parent_name)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="parent_whatsapp" class="block mt-1 w-full" type="text" name="parent_whatsapp"
                                    :value="old('parent_whatsapp', $applicant->parent_whatsapp)" placeholder="628xxxxxxxxxx" />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="address" name="address"
                                    class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full"
                                    rows="3">{{ old('address', $applicant->address) }}</textarea>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="notes" name="notes"
                                    class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full"
                                    rows="2">{{ old('notes', $applicant->notes) }}</textarea>
