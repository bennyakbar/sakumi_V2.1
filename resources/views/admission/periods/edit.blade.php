<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('app.nav.admission_periods') }} - {{ __('app.button.edit') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admission.periods.update', $period) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="registration_open" class="block mt-1 w-full" type="date" name="registration_open"
                                    :value="old('registration_open', $period->registration_open?->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="registration_close" class="block mt-1 w-full" type="date" name="registration_close"
                                    :value="old('registration_close', $period->registration_close?->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <select id="status" name="status"
                                    ...
                                    required>
                                    <option value="draft" {{ old('status', $period->status) === 'draft' ? 'selected' : '' }}>{{ __('app.status.draft') }}</option>
                                    <option value="open" {{ old('status', $period->status) === 'open' ? 'selected' : '' }}>{{ __('app.status.open') }}</option>
                                    <option value="closed" {{ old('status', $period->status) === 'closed' ? 'selected' : '' }}>{{ __('app.status.closed') }}</option>
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="notes" name="notes"
                                    class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full"
                                    rows="3">{{ old('notes', $period->notes) }}</textarea>
    ...
    <script>
        function quotaManager() {
            return {
                rows: @json($period->quotas->map(fn ($q) => ['class_id' => (string) $q->class_id, 'quota' => (string) $q->quota])->values()),
