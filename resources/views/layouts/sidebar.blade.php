...
<div x-show="mobileOpen" x-transition:enter="transition-opacity ease-linear duration-300"
     ...
     @click="closeMobile()" class="fixed inset-0 z-40 bg-gray-600/50 lg:hidden" x-cloak></div>
...
>
    ...
    {{-- Navigation --}}
    <nav class="flex-1 overflow-y-auto py-4 space-y-1" style="scrollbar-width: none;">
        {{-- Dashboard --}}
        ...
        {{-- Finance Group --}}
        @if(auth()->user()->canAny(['transactions.view', 'invoices.view', 'settlements.view', 'expenses.view']))
            <x-sidebar-group :label="__('app.nav.finance')" :active="request()->routeIs('transactions.*') || request()->routeIs('invoices.*') || request()->routeIs('settlements.*') || request()->routeIs('expenses.*')">
        ...
        @can('bank-reconciliation.view')
            <x-sidebar-group :label="__('app.nav.reconciliation')" :active="request()->routeIs('bank-reconciliation.*')">
                <x-sidebar-link :href="route('bank-reconciliation.index')" :active="request()->routeIs('bank-reconciliation.*')">
                    <x-slot name="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0012 9.75c-2.551 0-5.056.2-7.5.582V21M3 21h18M12 6.75h.008v.008H12V6.75z" />
                        </svg>
                    </x-slot>
                    {{ __('app.nav.bank_recon') }}
                </x-sidebar-link>
            </x-sidebar-group>
        @endcan
        ...
        @if(auth()->user()->canAny(['admission.periods.view', 'admission.applicants.view']))
            <x-sidebar-group :label="__('app.nav.admission')" :active="request()->routeIs('admission.*')">
                @can('admission.periods.view')
                    <x-sidebar-link :href="route('admission.periods.index')" :active="request()->routeIs('admission.periods.*')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                @can('admission.applicants.view')
                    <x-sidebar-link :href="route('admission.applicants.index')" :active="request()->routeIs('admission.applicants.*')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        @if(
            ...
            <x-sidebar-group :label="__('app.nav.master_data')" :active="request()->routeIs('master.*')">
                @if(auth()->user()->hasAnyRole($masterDataCoreRoles) && auth()->user()->can('master.students.view'))
                    <x-sidebar-link :href="route('master.students.index')" :active="request()->routeIs('master.students.*')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                @if(auth()->user()->hasAnyRole($masterDataCoreRoles) && auth()->user()->can('master.classes.view'))
                    <x-sidebar-link :href="route('master.classes.index')" :active="request()->routeIs('master.classes.*')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                @if(auth()->user()->hasAnyRole($masterDataCoreRoles) && auth()->user()->can('master.categories.view'))
                    <x-sidebar-link :href="route('master.categories.index')" :active="request()->routeIs('master.categories.*')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        @if(auth()->user()->canAny(['reports.daily', 'reports.monthly', 'reports.arrears', 'reports.ar-outstanding', 'reports.collection', 'reports.student-statement', 'reports.cash-book']))
            <x-sidebar-group :label="__('app.nav.reports')" :active="request()->routeIs('reports.*')">
                @can('reports.daily')
                ...
                @can('reports.monthly')
                    <x-sidebar-link :href="route('reports.monthly')" :active="request()->routeIs('reports.monthly')">
                        ...
                        {{ __('app.nav.monthly_report') }}
                    </x-sidebar-link>
                @endcan

                @can('reports.arrears')
                    <x-sidebar-link :href="route('reports.arrears')" :active="request()->routeIs('reports.arrears')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                            </svg>
                        </x-slot>
                        {{ __('app.nav.arrears_report') }}
                    </x-sidebar-link>
                @endcan

                @can('reports.ar-outstanding')
                    <x-sidebar-link :href="route('reports.ar-outstanding')" :active="request()->routeIs('reports.ar-outstanding')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                            </svg>
                        </x-slot>
                        {{ __('app.nav.ar_outstanding') }}
                    </x-sidebar-link>
                @endcan

                @can('reports.collection')
                    <x-sidebar-link :href="route('reports.collection')" :active="request()->routeIs('reports.collection')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                            </svg>
                        </x-slot>
                        {{ __('app.nav.collection_report') }}
                    </x-sidebar-link>
                @endcan

                @can('reports.student-statement')
                    <x-sidebar-link :href="route('reports.student-statement')" :active="request()->routeIs('reports.student-statement')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5zm6-10.125a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm-3.375 6.166a3.001 3.001 0 015.003 1.459H6.377a3.001 3.001 0 01.748-1.459z" />
                            </svg>
                        </x-slot>
                        {{ __('app.nav.student_statement') }}
                    </x-sidebar-link>
                @endcan

                @can('reports.cash-book')
                    <x-sidebar-link :href="route('reports.cash-book')" :active="request()->routeIs('reports.cash-book')">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                            </svg>
                        </x-slot>
                        {{ __('app.nav.cash_book') }}
                    </x-sidebar-link>
                @endcan
            </x-sidebar-group>
        @endif
        ...
        @if(auth()->user()->canAny(['users.view', 'settings.view']))
            <x-sidebar-group :label="__('app.nav.settings')" :active="request()->routeIs('users.*') || request()->routeIs('settings.*')">
                @can('users.view')
    ...
    <div class="hidden lg:flex shrink-0 border-t border-gray-200 p-2">
        <button @click="toggleCollapse()" class="flex w-full items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors duration-150">
